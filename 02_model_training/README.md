...
2. `vectorize_stream(corpus, min_freq, dim, win, alg)` builds `FastText(vector_size=dim, window=win, min_count=min_freq, sg=(1 if alg == "sg" else 0), sample=0.0001, negative=10, alpha=0.05, min_n=3, max_n=6, workers=mt.workers)`, then calls `build_vocab` and `train` (10 epochs, matching the subs2vec parameters this project replicates -- see `05_manuscript/manuscript.Rmd`) on that same in-memory `corpus` list. `build_models` always calls this with `min_freq=5`, so words appearing fewer than 5 times in the corpus are dropped from the vocabulary; `min_n`/`max_n` set the character n-gram range fastText uses to build subword representations.
