...
affiliation:
  - id: 1
    institution: Computer and Information Sciences, Harrisburg University of Science and Technology, Harrisburg, United States of America
  - id: 2
    institution: Department of Psychology and Neuroscience, Boston College, Boston, United States of America
  - id: 3
    institution: Department of Cognition, Emotion, and Methods in Psychology, Faculty of Psychology, University of Vienna, Vienna, Austria
  - id: 4
    institution: Department of Psychology, The University of Alabama - Tuscaloosa, Tuscaloosa, United States of America
  - id: 5
    institution: Department of Psychology, Ashland University, Ashland, United States of America
  - id: 6
    institution: Department of Psychology, University of Birmingham, Birmingham, United Kingdom
  - id: 7
    institution: Department of Vision Sciences, University of Leicester, Leicester, United Kingdom
  - id: 8
    institution: Department of Psychology, Goethe University Frankfurt, Frankfurt, Germany
  - id: 9
    institution: Department of Psychology, Education and Child Studies, Erasmus University Rotterdam, Rotterdam, The Netherlands
  - id: 10
    institution: Department of Sport, Tourism and Hospitality Management, Temple University, Philadelphia, United States of America
  - id: 11
    institution: Department of Psychology, Cis_Iscte, University Institute of Lisbon, Lisbon, Portugal
  - id: 12
    institution: Department of Psychology, Brandenburg University of Technology Cottbus-Senftenberg, Cottbus-Senftenberg, Germany
  - id: 13
    institution: Royal Commission for Jubail and Yanbu, Jubail Industrial City, Saudi Arabia
  - id: 14
    institution: Department of Psychology, Bilkent University, Ankara, Turkey
  - id: 15
    institution: Faculty of Psychology, CONICET, University of Buenos Aires, Buenos Aires, Argentina
  - id: 16
    institution: Department of Psychology, University of Palermo, Buenos Aires, Argentina
  - id: 17
    institution: Analytics, Harrisburg University of Science and Technology, Harrisburg, United States of America
...
Figure \@ref(fig:tech-details) outlines the workflow for data acquisition, text preprocessing, corpus creation, and the generation of word-by-dimension matrices for each parameter combination. These procedures build on the original Python code from the subs2vec paper, with modifications tailored to the needs of this experiment. The full source code is available at <https://github.com/SemanticPriming/word2manylanguages>.
...
van Paridon and Thompson [-@vanparidon2021] applied sentence-level deduplication within each subtitle and Wikipedia document to reduce the influence of commonly repeated phrases. In contrast, we chose to retain these frequent phrases—such as "Thank you" because of their prevalence in spoken language, which we consider relevant to our analysis. We did apply document-level deduplication using simhash-based near-duplicate detection (documents within a Hamming distance of 2 were treated as duplicates and pruned) to avoid including duplicate or highly similar documents, though given the curated nature of our data sources, the likelihood of such duplication was low. One corpus file was produced per language, with each file containing one sentence per line.
...
We used fivefold ridge regression (*k* = 5), repeated ten times with random splits each time, to predict norm values from the embeddings. Ridge regression was chosen due to its effectiveness in mitigating multicollinearity which is a common issue in word embedding models [@kaveh-yazdy], and its demonstrated ability to improve mean squared error performance in this context [@yeh2020]. This matches the cross-validation procedure reported by van Paridon and Thompson [-@vanparidon2021] ("fivefold cross validation repeated ten times"). The ridge regression alpha parameter, which controls the regularization strength, was set to the default value of 1 to balance bias and variance, consistent with prior work.

Each model configuration was evaluated using both its raw output vectors and L2-normalized (unit-length) vectors. For evaluations against extended norms and replication datasets—whose ground-truth word lists are limited to a few hundred words—raw vectors at higher embedding dimensions (200 and above) produced numerically unstable, strongly negative $R^2$ values under the fixed regularization strength described above ($\alpha = 1$), an artifact of insufficient regularization relative to unnormalized feature scale in this low-sample, high-dimension setting. This instability was not observed for the frequency-count evaluation (Research Question 2), whose datasets span tens of thousands of words and provide enough samples for stable ridge fits regardless of vector scale. We therefore report results from L2-normalized vectors throughout; raw-vector results are provided in the supplemental materials for reference.
...
```{r ft-w2v-load, include=FALSE}
ft_w2v_dir <- "../experiments/fasttext_vs_word2vec/results"
timing_df <- dplyr::bind_rows(lapply(list.files(ft_w2v_dir, pattern = "_timing\\.csv$", full.names = TRUE), import))
rsa_df <- dplyr::bind_rows(lapply(list.files(ft_w2v_dir, pattern = "_rsa\\.csv$", full.names = TRUE), import))
pred_df <- dplyr::bind_rows(lapply(list.files(ft_w2v_dir, pattern = "_predictive\\.csv$", full.names = TRUE), import))

lang_names <- expand_lang(unique(timing_df$language)) %>%
  dplyr::rename(language_name = language, language = code)

timing_summary <- timing_df %>%
  tidyr::pivot_wider(id_cols = c(language, dim, window, alg), names_from = family, values_from = seconds) %>%
  dplyr::filter(!is.na(fasttext), !is.na(word2vec)) %>%
  dplyr::mutate(speedup = fasttext / word2vec) %>%
  dplyr::left_join(lang_names, by = "language")

rsa_ftvw2v <- rsa_df %>%
  dplyr::filter(comparison == "fasttext_vs_word2vec") %>%
  dplyr::left_join(lang_names, by = "language")

pred_counts <- pred_df %>%
  dplyr::filter(eval_type == "counts", normalized == TRUE) %>%
  dplyr::group_by(language, family) %>%
  dplyr::summarise(mean_r = mean(r, na.rm = TRUE), .groups = "drop") %>%
  tidyr::pivot_wider(names_from = family, values_from = mean_r) %>%
  dplyr::left_join(lang_names, by = "language")

n_ft_w2v_langs <- length(unique(timing_summary$language))
mean_speedup <- mean(timing_summary$speedup, na.rm = TRUE)
min_speedup <- min(timing_summary$speedup, na.rm = TRUE)
max_speedup <- max(timing_summary$speedup, na.rm = TRUE)
mean_rsa <- mean(rsa_ftvw2v$pearson_r, na.rm = TRUE)
```

Every model reported below was trained with fastText's subword-informed embeddings [@bojanowski2016], which represent each word as the sum of its constituent character n-gram vectors (minimum length three, maximum length six) rather than fitting a single vector per wordform as in the original word2vec algorithm [@mikolov2013]. This subword mechanism is intended to improve embeddings for morphologically rich languages and out-of-vocabulary words, but it comes at a real computational cost: gensim's fastText implementation must additionally hash and train vectors for every n-gram of every word, not just the wordform itself. Because the full 60-configuration sweep across all remaining languages was not yet complete when this manuscript was prepared, we conducted a smaller, targeted comparison to determine whether that added cost was justified before committing the full parameter sweep to it.

We reran a subset of five (dimension, window) configuration points -- (50, 1), (100, 2), (200, 3), (300, 4), (500, 6), each with both CBOW and skip-gram -- with plain word2vec in place of fastText, for `r n_ft_w2v_langs` languages selected to span the morphological and orthographic diversity of the full language sample: an isolating language with comparatively simple morphology (Afrikaans), agglutinative languages from three unrelated families (Basque, Kazakh, Tamil, Korean), fusional languages from two families (Hindi, Macedonian), and isolating/analytic languages, one of which requires word segmentation rather than whitespace tokenization (Thai, Vietnamese) -- across Latin, Cyrillic, Devanagari, Thai, and Hangul scripts. For every resulting model we recorded (a) training time, (b) representational similarity analysis (RSA) [@kriegeskorte2008] between the fastText and word2vec embedding spaces -- correlating each model's own pairwise cosine-similarity matrix over a shared sample of frequent words, since the two models' raw vector spaces are not otherwise directly comparable -- and (c) predictive power against the same frequency-count evaluation used in Research Question 2.

Figure \@ref(fig:ft-w2v-fig) summarizes all three comparisons. Word2vec trained substantially faster than fastText for every language and configuration tested, averaging a `r round(mean_speedup, 2)`x speedup (range `r round(min_speedup, 2)`--`r round(max_speedup, 2)`x across languages), with the largest gap at the highest dimension/window settings, where fastText's subword overhead compounds most. Despite this speed difference, the two algorithms' embedding spaces were highly correlated (mean Pearson $r$ = `r round(mean_rsa, 2)` across configurations and languages), indicating that fastText's subword mechanism does not fundamentally reorganize which words are considered similar to which, at least for the languages tested here. Predictive power against frequency counts was, if anything, slightly higher for word2vec than fastText in every language tested. Taken together, these preliminary results suggest that fastText's subword mechanism -- and its associated training cost -- may not be necessary for the languages and evaluation tasks considered in this study; we return to this point, and to whether the remaining parameter sweep should therefore be conducted with word2vec instead, in the Discussion.

```{r ft-w2v-fig, fig.height=4, fig.width=10, fig.cap="Comparison of fastText and word2vec for a targeted subset of languages spanning diverse morphology and scripts. (A) Distribution of word2vec speedup (fastText seconds / word2vec seconds) across the five (dimension, window) configurations tested, by language; values above the dashed line at 1 indicate word2vec trained faster. (B) Representational similarity analysis (RSA) Pearson correlation between the fastText and word2vec embedding spaces at each configuration, by language. (C) Mean predictive power (Pearson r) against frequency-count norms for L2-normalized vectors, paired by language and algorithm."}
fig.timing <-
  ggplot(timing_summary, aes(x = reorder(language_name, speedup, FUN = mean), y = speedup)) +
  geom_boxplot(fill = "grey85") +
  geom_hline(yintercept = 1, linetype = "dashed", color = "grey40") +
  coord_flip() +
  labs(x = NULL, y = "word2vec speedup (x)", title = "A. Training time") +
  theme_minimal(base_size = 11)

fig.rsa <-
  ggplot(rsa_ftvw2v, aes(x = reorder(language_name, pearson_r, FUN = mean), y = pearson_r)) +
  geom_boxplot(fill = "grey85") +
  coord_flip() +
  labs(x = NULL, y = "Pearson r", title = "B. Embedding agreement (RSA)") +
  theme_minimal(base_size = 11)

fig.pred <-
  ggplot(pred_counts, aes(y = reorder(language_name, word2vec))) +
  geom_segment(aes(x = fasttext, xend = word2vec, yend = language_name), color = "grey60") +
  geom_point(aes(x = fasttext, color = "fastText"), size = 3) +
  geom_point(aes(x = word2vec, color = "word2vec"), size = 3) +
  scale_color_manual(values = c("fastText" = "#440154", "word2vec" = "#21918c"), name = NULL) +
  labs(x = "Mean r (frequency counts)", y = NULL, title = "C. Predictive power") +
  theme_minimal(base_size = 11) +
  theme(legend.position = "bottom")

fig.timing + fig.rsa + fig.pred
```
...
# heat map 
df1.both <- import("../results/formatted_data/rep_evals_formatted.csv") %>% 
...
The second set of tests addressed a key limitation: the lack of normed datasets for many of the languages modeled in this and previous studies. While prior work did not evaluate all available languages, word frequency was available for all models, and word frequency is known to correlate with numerous linguistic phenomena [@brysbaert2009]. Unigram (i.e., single-token) frequency counts were directly extracted from the same corpora used to train the embeddings. However, this frequency data initially posed challenges: it included ligatures and diacritics that did not always align with the forms present in the word-by-dimension matrices, since the training corpora were never transliterated and so retained each language's native script and diacritics throughout. We matched words directly after case folding, then applied Unicode normalization and transliteration only as a fallback for words that did not match directly; applying transliteration unconditionally would have discarded a substantial share of otherwise-correct matches for languages using non-Latin scripts, since it converts a native-script word into a romanized form the embedding vocabulary never contains. Despite this two-stage matching, a substantial number of words remained unmatched, primarily due to the minimum frequency threshold of five tokens in the word embeddings, in contrast to no such threshold in the raw unigram frequency data.
...
# heat map 
df2.both <- import("../results/formatted_data/count_evals_formatted.csv") %>% 
...
# heat map 
df3.both <- import("../results/formatted_data/extension_evals_formatted.csv") %>% 
  group_by(language, algo) %>% 
  arrange(desc(`adjusted r-squared`)) %>% 
  slice_head(n = 3)
