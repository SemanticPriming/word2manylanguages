...
```{r}
count.files <- list.files(path = "../results/count_evals",
                          pattern = "*.csv",
                          full.names = TRUE)
...
export(count.full, "../results/formatted_data/count_evals_formatted.csv", row.names = F)
```

### Replication Tests `rep_evals`

```{r}
rep.files <- list.files(path = "../results/rep_evals",
                          pattern = "*.csv",
                          full.names = TRUE)
...
rep.full <- bind_rows(rep.dfs) %>% 
  ...
  mutate(source = rep(rep.files, times = lapply(rep.dfs, nrow)),
         source = gsub("../results/rep_evals/|_wxd_eval.csv|_eval.csv", "", source)) %>% 
  separate(source, into = c("language", "dim", "window", "algo"), sep = "_")

export(rep.full, "../results/formatted_data/rep_evals_formatted.csv", row.names = F)
```

### Extension Tests `extension_evals`

```{r}
extension.files <- list.files(path = "../results/extension_evals",
                          pattern = "*.csv",
                          full.names = TRUE)
...
export(extension.full, "../results/formatted_data/extension_evals_formatted.csv", row.names = F)
```